...
Rust formatting is deliberately *not* a git hook (per-file rustfmt chokes on
the Liquid templates) — use `fmt` or editor format-on-save. Linting/compile-
checking the ESP targets needs falcon, so it lives in `check` rather than a
local git hook.

## Editor / LSP (Helix, Zed)

Open **`embedded/`** as the project so its devenv (esp-rs toolchain +
rust-analyzer) is active. Configure rust-analyzer with each esp-rs board as a
`linkedProject` (RA uses one target per project) — e.g. in a local
`.helix/languages.toml` or `.zed/settings.json`.
Open `boards/arduino-uno-r3` separately — it uses the AVR toolchain.
