...
* **Editor / LSP** — open **`embedded/`** as your project (so its devenv +
  esp-rs toolchain are active). Configure rust-analyzer with one
  `linkedProject` per esp-rs board (RA uses one target per project) — e.g. in a
  local `.helix/languages.toml` or `.zed/settings.json`. The AVR board is opened separately (different toolchain).
