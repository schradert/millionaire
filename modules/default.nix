    ...
    config = lib.mkIf config.profiles.workstation.enable (lib.mkMerge [
      {
        ...
        home.file.".opencode/agents".source =
          pkgs.runCommand "agency-agents-patched" {
        ...
        home.packages = with pkgs;
          lib.mkMerge [
            ...
            (lib.mkIf stdenv.hostPlatform.isLinux [
              ...
              # shared list alongside bitwarden-desktop once nixpkgs' darwin
              # stdenv bootstrap is fixed
              picard
              siyuan
            ])
          ];
