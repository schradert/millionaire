...
#    a tiny closure. Falcon builds its own activation natively.
{
  ...
  profiles = let
    ...
    in
      ...
      then let chip = builtins.replaceStrings ["-"] [""] platform; in ''
        ...
        echo "[${name}] flashed"
      ''
      else if platform == "arduino-uno-r3" then ''
        PORT="/dev/serial/by-id/${cfg.usbSerial}"
        if [ ! -e "$PORT" ]; then
          echo "[${name}] not connected at $PORT, skipping"; exit 0
        fi
      ...
      ''
      else if platform == "stm32-nucleo" then ''
        if ! direnv exec ${embeddedDir} probe-rs list 2>/dev/null | grep -q '.'; then
        ...
        echo "[${name}] flashed"
      ''
      else if platform == "teensy-4.1" then ''
        cd ${embeddedDir}
        echo "[${name}] building"
